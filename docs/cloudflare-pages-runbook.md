...
These are operator-controlled provider changes. The repository neither reads nor mutates the live Supabase Auth URL configuration, so record the actual dashboard values and acceptance results separately without copying one-time codes, PKCE values, or customer data into repository evidence.
...
2. Before changing the Site URL, add Redirect URL entries scoped to the exact `https://www.bncconsulting.nl/auth/callback` path **and its permitted query-string variants**. The application preserves a validated `next` value; automatic PKCE flow-ID redirects remain disabled until this verification passes, but enabling them later appends `sb_flow_id`. Use Supabase’s Redirect URL glob tester to verify that the least-broad callback-path pattern covers the bare callback, `?next=%2Faccount`, and `?next=%2Faccount&sb_flow_id=synthetic-flow-id`. Do not use a host-wide wildcard or allow provider/preview aliases. Retain equivalently query-aware `https://www.clearstep-ai.nl/auth/callback` entries only for the recorded drain/rollback window; allowing them does not transfer origin-scoped PKCE state through an HTTP redirect.
3. While the old Site URL is still configured, test fresh email and Google sign-in on the BNC host and confirm in browser/network evidence that Supabase keeps the requested BNC callback, including `next` and, when emitted, `sb_flow_id`, instead of falling back to the old Site URL. Redact the authorization code and real flow identifier. Only after those checks pass, set the Supabase Auth Site URL to `https://www.bncconsulting.nl` and repeat both sign-in tests.
...
- fresh magic-link and Google sign-in return through the canonical Auth callback with `next` and, when emitted, `sb_flow_id` preserved without recording their live values; staff/admin access, private quote/invite links, test Checkout success/cancel flows for both a Clearstep workshop and a published Plate & Post service, and signed webhook processing pass;
