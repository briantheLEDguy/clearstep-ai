...
| Service orders and fulfilment | customer email, purchased package/amount, payment and staff-managed fulfilment status | contract and pre-sale information, manual scheduling/contact process, cancellation/refund handling, retention, role-based staff access |
...
| Customer rights and purchase changes | authenticated request, selected workshop enrollment or service order, limited explanatory text, review/audit status | identity and ownership checks, manual decision, response process, retention, staff access |
...
- Confirm the versions in `shared/legal-documents.ts` match the text shown on the policy pages and the versions recorded with checkout acknowledgements. Issue a new version and record the release review whenever customer-facing policy wording changes. Cloudflare Pages publication does not attest the deployed checkout Function bundle, so record the matching Function deployment/version as separate evidence before launch.
- Confirm provider agreements, data-processing roles, international-transfer safeguards, and the rights-request process for Supabase, Stripe, Cloudflare Pages, and Google Workspace where applicable.
- Before publishing a Plate & Post package, confirm that the displayed gross price, approved tax/VAT treatment, deliverables, manual-scheduling statement, cancellation/refund wording, and staff follow-up process match the actual service. A Stripe Price with `tax_behavior=inclusive` while automatic tax is disabled does not by itself calculate or show VAT: obtain finance/tax approval for any manual inclusive Tax Rate and verify the Checkout/invoice breakdown. Record test-mode evidence that the Product/Price belongs to the existing BNC Stripe account and that Checkout, signed webhooks, customer ownership, email, refund, and fulfilment paths behave as reviewed. Do not treat test objects as live-mode approval.
- For the BNC domain migration, verify that the canonical domain, privacy/contact links, Supabase Auth callback, generated customer links, and Stripe return paths agree. Prefix old Clearstep marketing paths, preserve only the documented shared customer/staff paths, send unknown old paths to `/clearstep/`, and retain every query; never log or include one-time token URLs in public release evidence.
...
- anonymous-versus-staff catalogue evidence showing draft Plate & Post offerings remain private, plus customer ownership and role-gated service-order/fulfilment evidence after a package is published;
- configuration confirmation that secret values are stored only in their provider and that Cloudflare Pages receives public build values only;
- rendered response-header evidence for the canonical host, noindex evidence for provider aliases, and redirect/TLS checks for the apex and old Clearstep host.

Cloudflare Pages applies the repository’s static `_headers` rules, but those browser controls are defense in depth rather than an authorization boundary. Retain RLS, authenticated Edge Function authorization, webhook verification, and provider-side access controls. Review the deployed headers after every material source/provider change, especially before tightening the CSP. Route suspected vulnerabilities through [SECURITY.md](../SECURITY.md); do not put confidential evidence in a public issue.
