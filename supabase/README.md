...
The browser’s Auth callback can include a validated `next` query parameter. Automatic PKCE flow-ID redirects are intentionally disabled until the live allowlist is verified; enabling them later can append `sb_flow_id`. Before the BNC host becomes the Supabase Auth Site URL, an exact queryless Redirect URL entry may not cover those full callback variants and Auth can fall back to the old Site URL. Validate the least-broad pattern scoped to the exact `/auth/callback` path with Supabase’s Redirect URL glob tester; cover the bare callback and permitted query forms without adding a host-wide wildcard or provider/preview aliases. These dashboard changes are operator actions and are not made or proven by this repository.

1. Serve the validated application over HTTPS at `https://www.bncconsulting.nl` before changing generated links.
2. Add BNC callback-path Redirect URL entries that match `https://www.bncconsulting.nl/auth/callback`, `?next=%2Faccount`, and `?next=%2Faccount&sb_flow_id=synthetic-flow-id`. Retain equivalently query-aware old-host callback entries for the recorded transition/rollback window. While the old Site URL remains configured, confirm Supabase preserves the requested BNC callback instead of falling back to the old host; redact real authorization codes and flow identifiers from evidence. Change the Auth Site URL only after fresh email and Google sign-in tests pass, then repeat them after the change.
...
5. Test a fresh magic link and Google sign-in through the canonical callback with `next` and, when emitted, `sb_flow_id` preserved, plus a staff invitation, private quote, checkout success/cancel path, and signed webhook delivery. Existing browser sessions and PKCE state are origin-scoped, so require a fresh sign-in on the BNC domain rather than attempting to transfer them.
